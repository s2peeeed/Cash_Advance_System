...
// Handle form submission for adding new position
    ...
    if (isset($_POST['add_position'])) {
        ...
        if ($position_name) {
            try {
                ...
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM positions WHERE position_name = ?");
                $stmt->execute([$position_name]);
                if ($stmt->fetchColumn() > 0) {
                    $error = "Position name already exists. Please use a different name.";
                } else {
            ...
            $error = "Please enter a position name.";
        }
    } elseif (isset($_POST['update_position'])) {
        $position_name = trim($_POST['position_name']);
        $original_position_id = $_POST['original_position_id'];
        
        if ($position_name) {
            try {
                // Check for duplicate position name (excluding current position)
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM positions WHERE position_name = ? AND id != ?");
                $stmt->execute([$position_name, $original_position_id]);
                if ($stmt->fetchColumn() > 0) {
                    $error = "Position name already exists. Please use a different name.";
                } else {
                    ...
                    $edit_position = null; // Clear edit mode
                }
            } catch (PDOException $e) {
                $error = "Database error: " . $e->getMessage();
            }
        ...
        try {
            // Check if position is being used in any bonded_employees
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM bonded_employees WHERE position = (SELECT position_name FROM positions WHERE id = ?)");
            $stmt->execute([$position_id]);
            $usage_count = $stmt->fetchColumn();
            
            if ($usage_count > 0) {
                $error = "Cannot delete position '" . htmlspecialchars($position_name) . "'. It is being used by " . $usage_count . " bonded employee(s). Please update those records first.";
            } else {
                $stmt = $pdo->prepare("DELETE FROM positions WHERE id = ?");
                if ($stmt->execute([$position_id])) {
                    $success = "Position '" . htmlspecialchars($position_name) . "' has been deleted successfully!";
                } else {
...
// Fetch all positions with usage count
$positions = [];
try {
    $stmt = $pdo->query("
        SELECT p.*, 
        ...
        FROM positions p
        LEFT JOIN (
            SELECT position, COUNT(*) as usage_count 
            FROM bonded_employees 
            GROUP BY position
        ) usage_stats ON p.position_name = usage_stats.position
...
<head>
    ...
    <title>Position Management - LGU Liquidation System</title>
...
<body>
                ...
                <div class="main-card animate__animated animate__fadeIn">
                    <div class="card-body">
                        ...
                        <div class="page-header">
                            <h1 class="page-title">
                                <i class="fas fa-briefcase me-2"></i>Position Management
                            </h1>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr class="fade-in">
                                        <td>
                                            <strong>#<?php echo htmlspecialchars($position['id']); ?></strong>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <?php if ($position['usage_count'] > 0): ?>
                                                <span class="badge badge-info">
                                                    <i class="fas fa-users me-1"></i><?php echo $position['usage_count']; ?> employee(s)
                                                </span>
                                            <?php else: ?>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                <form method="POST" style="display: inline;">
                                                    ...
                                                    <button type="submit" name="edit_position" class="btn btn-sm btn-primary" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                </form>
                                                <form method="POST" style="display: inline;" onsubmit="return confirmDelete('<?php echo htmlspecialchars($position['position_name']); ?>', <?php echo $position['usage_count']; ?>)">
                                                    <input type="hidden" name="position_id" value="<?php echo htmlspecialchars($position['id']); ?>">
                                                    <input type="hidden" name="position_name" value="<?php echo htmlspecialchars($position['position_name']); ?>">
                                                    <button type="submit" name="delete_position" class="btn btn-sm btn-danger" title="Delete" <?php echo $position['usage_count'] > 0 ? 'disabled' : ''; ?>>
                ...
                <form method="POST">
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="position_name" name="position_name" required placeholder="e.g., Department Head, Staff, Manager">
                        </div>
                    </div>
    ...
    <div class="modal fade" id="editPositionModal" tabindex="-1" aria-labelledby="editPositionModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="POST">
                    <input type="hidden" name="original_position_id" value="<?php echo htmlspecialchars($edit_position['id']); ?>">
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="edit_position_name" class="form-label">Position Name</label>
                            <input type="text" class="form-control" id="edit_position_name" name="position_name" value="<?php echo htmlspecialchars($edit_position['position_name']); ?>" required>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Show edit modal if edit_position is set
            <?php if ($edit_position): ?>
            ...
            // Handle edit modal
            const editModalElement = document.getElementById('editPositionModal');
            if (editModalElement) {
        ...
        // Delete confirmation
        function confirmDelete(positionName, usageCount) {
            if (usageCount > 0) {
                alert(`Cannot delete position "${positionName}" because it is being used by ${usageCount} employee(s). Please update those records first.`);
                return false;
            }
            return confirm(`Are you sure you want to delete position "${positionName}"? This action cannot be undone.`);
        }
        ...
        const addForm = document.querySelector('#addPositionModal form');
        if (addForm) {
        ...
        document.getElementById('edit_position_name')?.addEventListener('input', function() {
            if (this.value.trim()) {
                this.classList.remove('is-invalid');
            } else {
                this.classList.add('is-invalid');
